                    ...
                    <form method="POST" action="online/add" class="change-form" enctype="multipart/form-data">
                        ...
                        <div class="input-group">
                            <span class="input-group-btn">
                              <a id="btn-img" data-input="thumbnail" data-preview="box-preview-img"
    ...
    <div class="table-responsive">
        <table class="table table-bordered text-center" id="data_news">
            ...
            @foreach ($online as $courses)
                <tr>
                    <td class="btn-gr d-none"><input type="checkbox" class="check" name="delete[]"
                                                     value="{{ $courses->id }}"></td>
                    <td>@php
                    ...
                    <td>
                        <div class="btn-group-sm btn-func">
                            <button type="button" class="btn btn-warning" data-toggle="modal"
                                    data-target="#editModal{{$courses->id}}">
                                <span class="fas fa-edit"></span>
                            </button>
                            <a href="{{ url('online/delete') }}/{{$courses->id}}" class="btn btn-primary"
        ...
        <div class="modal fade" id="editModal{{$courses->id}}">
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="POST" action="{{ url('online/edit') }}/{{$courses->id}}" class="change-form"
                              enctype="multipart/form-data">
                            @csrf
                            <div class="input-group">
                                        <span class="input-group-btn">
                                            <a id="btn-edit-img" data-input="thumbnail-edit" data-preview="edit-img"
                                               class="btn btn-outline-primary">
                                            Chọn tệp:
                                            </a>
                                        </span>
                                <input class="form-control thumbnail-edit" type="text" name="filepath"
                                       value="{{ $courses->img }}">
                            </div>
                            ...
                            <label for="" class="required control-label">@lang('modules.online.content')</label>
                            <textarea class="edit-detail"
                                      name="detail">{!! old('detail', $courses->detail) !!}</textarea>
                            <label for="" class="required control-label">@lang('modules.online.level')</label>
                            <select class="js-level form-control" name="level">
                                @foreach ($level as $item)
                                ...
                                @foreach (explode(",", $courses->video) as $item)
                                    <div class="video">
                                        <textarea class="form-control" name="video[]"
                                                  placeholder="Đường link video...">{{$item}}</textarea>
                                        <hr>
                                    </div>
                                @endforeach
                            </div>
                            <a class="d-block add-video"
                               style="font-family: font-medium; color:2e1503; cursor: pointer;">Thêm...</a>
